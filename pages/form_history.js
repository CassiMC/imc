...
export default function FormHistory() {
    ...
    const rows = sheetInformation();

    const tab = (infos, infosLenght) =>{
        for (let i=0; i< infosLenght; i++ ) {
            line(infos[i]);
        }
    }
  ...
  return (
    <>
      <Head>
        <title>form_history</title>
      </Head>
      <div>
      <table>
        {tab(rows, rows.length)}
    </table>
    <h2>
        <Link href="/">
          <a>Back to home</a>
